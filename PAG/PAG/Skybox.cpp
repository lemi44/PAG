...
void Skybox::init(Shader* shader)
{
	...
	glBufferData(GL_ARRAY_BUFFER, 36 * 3 * sizeof(float), &skyboxVertices[0], GL_STATIC_DRAW);
...
void Skybox::loadCubemap(std::vector<std::string> faces)
{
	...
	for (unsigned int i = 0; i < faces.size(); i++)
	{
		...
		{
			Logger::logError("Cubemap texture failed to load at path: " + faces[i]);
			stbi_image_free(data);
		}
	}
...
void Skybox::drawSkybox(glm::mat4 const wvp)
{
